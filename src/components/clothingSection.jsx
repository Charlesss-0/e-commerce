			...
			{items.map(item => (
				<div key={item.id} className="flex items-center">
					<div className="text-center">
						<div className="overflow-hidden w-[200px] h-[220px] m-[1rem] rounded-lg shadow-lg hover:cursor-pointer hover:child-flex relative">
							<img
								src={item.img}
								alt={item.title}
								className="w-full h-full hover:scale-105 transition-all duration-300"
							/>
							<div className="gap-[1rem] absolute bottom-0 left-[50%] translate-x-[-50%] mb-[0.5rem] hidden [&>i]:shadow-md [&>i]:bg-[#efefef] [&>i]:p-[0.5rem] [&>i]:rounded-lg">
								<i className="fi fi-rr-heart hover:opacity-[0.9]"></i>
								<i className="fi fi-rr-plus hover:opacity-[0.9]"></i>
							</div>
						</div>
						<h1 className="mb-[0.5rem] w-[200px] m-auto">{item.info}</h1>
						<p className="text-[0.7rem]">USD {item.price}</p>
					</div>
